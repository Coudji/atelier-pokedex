...
export default class PokedexDetail extends HTMLElement {
	...
	render() {
		...
		const typesMarkup = pokemonData.type
			.map((type) => {
				return `
                <div>
                    <img src="${type.image}">
		...
		const evolutionMarkup = Object.keys(evolutionLevels)
			.map((level) => {
				return `
                <div>
                    <h3>Level ${level}</h3>
		...
		const displayMarkup = `
            ...
            <div class="detailModal">
                <img src=${pokemonData.image} alt="Image de ${
			pokemonData.name
		}" class="img-pokemon">
                <div class="infoBox">
                    <div class="stats">
            ...
            <div class="comment">
                    <p class="showComment">${
											pokemonData.comment ? pokemonData.comment : 'Espace commentaire'
										}</p>
